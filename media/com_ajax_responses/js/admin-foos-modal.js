...
(function () {
  ...
  /**
    ...
    * View element calls jSelectAjax_response when a ajax_response is clicked
    * jSelectAjax_response creates the link tag, sends it to the editor,
    * and closes the select frame.
    */

  window.jSelectAjax_response = function (id, title, catid, object, link, lang) {
    var hreflang = '';

    if (!Joomla.getOptions('xtd-ajax_responses')) {
      // Something went wrong
      window.parent.Joomla.Modal.getCurrent().close();
      return false;
    }

    var _Joomla$getOptions = Joomla.getOptions('xtd-ajax_responses'),
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    for (var i = 0, l = elements.length; l > i; i += 1) {
      ...
      elements[i].addEventListener('click', function (event) {
        ...
        if (functionName === 'jSelectAjax_response') {
          // Used in xtd_ajax_responses
